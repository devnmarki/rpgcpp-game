...
{
	getWorld()->query<TransformComponent, MovementComponent>([&](auto entity, TransformComponent& transform, MovementComponent& movement) {
		transform.position += movement.velocity * movement.movementSpeed * dt;
	});

	getWorld()->query<MovementComponent, InputComponent>([](auto entity, MovementComponent& movement, InputComponent& input) {
